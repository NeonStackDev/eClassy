...
class WalletController extends Controller
{
    ...
    public function transactionApprove(Request $request, $id)
    {
        ...
        try {
            $walletTransction = WalletTransaction::findOrFail($id);            
    ...
    public function transactionReject(Request $request, $id)
    {
        ...
        try {
            $walletTransction = WalletTransaction::findOrFail($id);            
            $walletTransction->status = 'rejected';
            $walletTransction->save();            
            return response()->json(['data'=>$walletTransction,'success'=>true,'message'=>'Reject Successful']);
            
        } catch (Throwable $th) {
            ResponseService::logErrorResponse($th, "WalletController ->  transactionReject");
            ResponseService::errorResponse();
        }
    }
    ...
    {
        ResponseService::noPermissionThenRedirect('user-wallet-list');
        return view('wallets.user');
    }

    public function userWalletsShow(Request $request)
    {
        ResponseService::noPermissionThenSendJson('user-wallet-list');
        $offset = $request->offset ?? 0;
        $limit = $request->limit ?? 10;
        $sort = $request->sort ?? 'id';
        $order = $request->order ?? 'DESC';

        $sql = Wallet::with('user:id,name');
        if (!empty($request->search)) {
            $search =  $request->search;
            $sql = $sql->whereHas('user', function ($query) use ($search) {
    ...
    public function paymentHistoryIndex()
    {
        ...
        ResponseService::noPermissionThenRedirect('user-wallet-hostory-list');
        return view('wallets.payment-histories');
    }

    public function paymentHistoryShow(Request $request)
    {
        ResponseService::noPermissionThenSendJson('user-wallet-hostory-list');
        $offset = $request->offset ?? 0;
        $limit = $request->limit ?? 10;
        $sort = $request->sort ?? 'id';
        $order = $request->order ?? 'DESC';
        $sql = WalletTransaction::with(['wallet.user'])->orderBy($sort, $order);
        if (!empty($request->search)) {
            $search =  $request->search;
            $sql = $sql->when($search, function ($query, $search) {
        ...
        foreach ($result as $key => $row) {
            ...
            $tempRow['proof_url'] = Storage::url($tempRow['proof_url']);
            $rows[] = $tempRow;
        }
